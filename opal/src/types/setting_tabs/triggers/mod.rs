...
use crate::{
    components::trigger_options::{self, Msg as TMsg},
    AsInputType, AsSettingOption, CallbackMsg, SettingCallbackFn, SettingOption, TotalMsgScope,
    ValueOP,
};
...
impl SettingCallbackFn<TMsg> for Msgs {
    fn msg_fn() -> Box<dyn Fn(Self) -> TMsg> {
        let f = |x| -> TMsg {
            match x {
